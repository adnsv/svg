		...
		case 'a':
			...
			for len(values) >= 7 {
				...
				/*
					...
					cur := last
					d := vertexMul(vertexDiff(cur, pt), 0.5)
					if d.X*d.X+d.Y*d.Y < 1e-12 || r.X < 1e-6 || r.Y <= 1e-6 {
						// too small? degenerate to a line
						pd.LineTo(pt)
					} else {
						sinTh, cosTh := math.Sincos(axisRotation)
						axis := Vertex{cosTh, sinTh}
						d = vertexRotate(d, axis)
						rr := Vertex{r.X * r.X, r.Y * r.Y}
						dd := Vertex{d.X * d.X, d.Y * d.Y}

						// check if radii are large enough
						check := dd.X/rr.X + dd.Y/rr.Y
						if check > 1.0 {
							m := 1.0 / math.Sqrt(check)
							r.X *= m
							r.Y *= m
						}

						// convert into transformed coordinate space
						v0 := vertexRotate(cur, axis)
						v1 := vertexRotate(pt, axis)
						v0.X /= r.X
						v0.Y /= r.Y
						v1.X /= r.X
						v1.Y /= r.Y
						...
						// calc circle center
						sfactorSq := 1.0/vertexNorm(vertexDiff(v1, v0)) - 0.25
						sfactor := float64(0)
						if sfactorSq > 0 {
						...
						c := vertexAdd(vertexMul(vertexAdd(v0, v1), 0.5), vertexMul(vertexTransDir(vertexDiff(v1, v0)), sfactor))

						th := vertexAtan2(vertexDiff(v0, c))
						thArc := vertexAtan2(vertexDiff(v1, c)) - th
						if thArc < 0 && sweep {
							thArc += 2.0 * math.Pi
						} else if thArc > 0 && !sweep {
							thArc -= 2.0 * math.Pi
						}
						nSegments := int(math.Ceil(math.Abs(thArc / (math.Pi*0.5 + 0.001))))
						if nSegments < 1 {
							nSegments = 1
						}

						for i := 0; i < nSegments; i++ {
							pd.arcSegment(c.X, c.Y, th+float64(i)*thArc/float64(nSegments), th+float64(i+1)*thArc/float64(nSegments), r.X, r.Y, axisRotation)
						}
						if cpt[2] != pt {
							pd.LineTo(pt)
						}
...
func vertexRotate(v, axis Vertex) Vertex {
	return Vertex{
		axis.X*v.X + axis.Y*v.Y,
		-axis.Y*v.X + axis.X*v.Y}
}

func vertexRevRotate(v, axis Vertex) Vertex {
	return Vertex{
		axis.X*v.X - axis.Y*v.Y,
		axis.Y*v.X + axis.X*v.Y}
}
...
func vertexDiff(a, b Vertex) Vertex {
	return Vertex{a.X - b.X, a.Y - b.Y}
}

func vertexNorm(v Vertex) float64 {
	return v.X*v.X + v.Y*v.Y
}

func vertexTransDir(v Vertex) Vertex {
	return Vertex{-v.Y, v.X}
}

func vertexMul(v Vertex, f float64) Vertex {
	return Vertex{v.X * f, v.Y * f}
}

func vertexScale(a, b Vertex) Vertex {
	return Vertex{a.X * b.X, a.Y * b.Y}
}

func vertexAtan2(v Vertex) float64 {
	return math.Atan2(v.Y, v.X)
}

func ellipseCenter(ra, rb *float64, rotX, startX, startY, endX, endY float64, sweep, smallArc bool) (cx, cy float64) {
